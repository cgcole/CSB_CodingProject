...
#Access files located in the data and input folders that will be analyized
data_files <- dir("../data", 
...
#For loop that cleans and generates graphs for input files
for (file in data_files) {
  ...
  if (sum(str_detect(file, plot_directory)) == 0) {
    ...
    #Create the directory that the plots will go into
    ...
    ggplot(data = growth_curve_data_final) + 
      ...
      ggsave(paste0("../plots/", test_number,"/overview_plot.eps"), 
             device = "eps",
             width = 26,
             height = 14,
             units = "cm")
    
    if (length(unique(growth_curve_summary$Strain)) > 1) {
      growth_curve_summary <- growth_curve_summary %>% 
        unite(Plasmid,
              Strain, Plasmid, 
              sep = " + ", 
              remove = FALSE,
    ...
    if (length(unique(growth_curve_summary$Media)) > 1) {
      ...
      ggplot(data = growth_curve_summary) + 
        ...
        ggsave(paste0("../plots/", test_number,"/main_plot.eps"), 
               device = "eps",
               width = 26,
               height = 14,
               units = "cm")
